...
import { KanbanType } from "@/types/context";
...
const KanbanProvider = ({ children }: ChildrenType) => {
  const [sections, setSections] =
    useState<KanbanSectionType[]>(kanbanSectionsData);
  const [tasks, setTasks] = useState<KanbanTaskType[]>(kanbanTasksData);
  const [activeSectionId, setActiveSectionId] =
    useState<KanbanSectionType["id"]>();
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    setTasks([
      ...newTasks.slice(0, destinationId + parity),
      task,
      ...newTasks.slice(destinationId + parity),
    ]);
  };
